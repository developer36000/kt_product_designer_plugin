...
class KTAdminProductActionsModel extends BaseAdminProductController {
	...
	public  function kt_add_action_button( $actions, $post ){
		...
		if( get_post_type() === $this->post_product ) {
			...
			$actions['make_as_default'] = '<a class="make_as_default_action" href="' . esc_url( $url ) . '"
		data-post_id="'.$post->ID.'">'.($is_super_product ? 'Unmark as Default' : 'Mark as Default').'</a>';
			if ( $is_super_product ) {
				echo '<style>
					...
					#post-'.$post->ID.' .check-column { pointer-events: none; opacity: 0.5; }
				</style>';
			} else {
				echo '<style>
	...
	public function ajax_action() {
		...
		} else if ($is_super_product) {
			update_post_meta($post_id, 'super_product', false );
		}
		echo $is_super_product;
		exit;
	}
	...
	public function ktwc_filter_product_meta_select() {
		...
		if ( $typenow == 'product' ) {
			$current_plugin = '';
			if( isset( $_GET['ADMIN_FILTER_STATUS_SLUG'] ) ) {
				$current_plugin = $_GET['ADMIN_FILTER_STATUS_SLUG']; // Check if option has been selected
			} ?>
	...
	public function ktwc_sort_product_by_meta( $query ) {
		...
		$current_select = $_GET['ADMIN_FILTER_STATUS_SLUG'];
		if ( is_admin() &&
		     'edit.php' == $pagenow &&
